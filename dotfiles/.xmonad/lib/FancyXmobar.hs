...
underline p c s = "<box type=Bottom width=" ++ show p
                  ++ " color=" ++ color ++ ">"
                  ++ s ++ "</box>"
...
fancy fg bg s = FancyString (colored fg bg s) fg bg
...
glue pp (FancyString t l r) (FancyString t' l' r')
  | r /= l' = FancyString res l r'
    where res = t ++ colored r l' (diffSeparator pp) ++ t'
  | r == l' = FancyString res l r
    where res = t ++ colored (backgroundColor pp) l' (sameSeparator pp) ++ t'
...
fancyWorkspaces sort' urgents pp s =
  foldl glue pp . map format . sort' . filter notNSP $ workspaces
  where
...
fancyTabs :: String -> Maybe NamedWindow -> [NamedWindow] -> FancyPP -> Seg
fancyTabs l f ws pp = glue pp $ prefix $ joinSep view postfix
  where
